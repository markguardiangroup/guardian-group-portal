...
function ComplianceBadge({ summary }: { summary?: ComplianceSummary }) {
  ...
  const complianceStyle: React.CSSProperties = { minWidth: '4.25rem', justifyContent: 'center' };

  if (score >= 90) {
    return (
      <Badge variant="outline" className="text-xs gap-1 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800" style={complianceStyle}>
  ...
  if (score >= 70) {
    return (
      <Badge variant="outline" className="text-xs gap-1 bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-200 dark:border-amber-800" style={complianceStyle}>
  ...
  return (
    <Badge variant="outline" className="text-xs gap-1 bg-red-500/10 text-red-600 dark:text-red-400 border-red-200 dark:border-red-800" style={complianceStyle}>
...
function SiteDocumentsModulePicker({ site }: { site: SiteWithDetails }) {
  const [, navigate] = useLocation();
  const { setSelectedSiteId, setSelectedCompany } = useSiteFilter();
  const [open, setOpen] = useState(false);
  const enabled = SITE_COMPLIANCE_MODULES.filter(m => site.moduleAccess?.[m.moduleKey] !== "hidden");
  ...
  if (enabled.length <= 1) {
    const dest = enabled.length === 1 ? enabled[0].path : "/health-safety";
    return (
      <span role="button" className="cursor-pointer" onClick={(e) => { e.stopPropagation(); goTo(dest); }} data-testid={`badge-doccount-${site.id}`}>
        {docsBadge}
      </span>
    );
  }
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <span role="button" className="cursor-pointer" onClick={(e) => e.stopPropagation()} data-testid={`badge-doccount-${site.id}`}>
          {docsBadge}
        </span>
      </PopoverTrigger>
      <PopoverContent className="w-52 p-1.5" align="start" onClick={(e) => e.stopPropagation()}>
