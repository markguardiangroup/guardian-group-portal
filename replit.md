...
A **Company Required Templates** feature allows admins to designate which private document templates are required for compliance at a company level. This is managed via a `company_required_templates` join table (companyId ↔ templateId). The company detail page includes a "Required Documents" card with per-module tabs showing checkboxes for private templates. Missing required templates at a site count as overdue in the site compliance summary, lowering the compliance score.
