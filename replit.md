...
How to add an entry:
- Append a new object to `versions[active].entries` in `changelog.json` with: `id` (uuid), `patch` (current `patch` of the active version), `message`, `category` (`bug` | `enhancement` | `feature` | `other`), `createdAt` (ISO timestamp), `createdBy` (`"System"`).
