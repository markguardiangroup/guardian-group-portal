...
export async function registerRoutes(
  ...
  // Patch document template visibility (isPublic + toolkitFolderId) — admin/consultant only
  app.patch("/api/document-templates/:id/visibility", requireAuth, async (req, res) => {
    try {
      const user = await storage.getUser((req.session as any).userId);
      if (!user) return res.status(401).json({ error: "User not found" });
      ...
      const { isPublic, toolkitFolderId } = req.body;

      const updated = await storage.updateDocumentTemplate(req.params.id, {
        isPublic: typeof isPublic === "boolean" ? isPublic : template.isPublic,
        toolkitFolderId: toolkitFolderId !== undefined ? (toolkitFolderId || null) : template.toolkitFolderId,
      } as any);
  ...
  // Toolkit — folders (all authenticated users)
  app.get("/api/toolkit/folders", requireAuth, async (req, res) => {
    try {
      ...
      const toolkitFolders = allFolderTemplates.filter((f: any) => f.module === "toolkit" && f.isActive);
      res.json(toolkitFolders);
    } catch (error) {
      console.error("Toolkit folders error:", error);
      res.status(500).json({ error: "Failed to fetch toolkit folders" });
    }
  });

  // Toolkit — templates (clients see public only; admins/consultants see all)
  app.get("/api/toolkit/templates", requireAuth, async (req, res) => {
    try {
      const user = await storage.getUser((req.session as any).userId);
      if (!user) return res.status(401).json({ error: "User not found" });
      ...
      const allTemplates = await storage.getDocumentTemplates();
      const allFolderTemplates = await storage.getFolderTemplates();
      const folderMap = new Map(allFolderTemplates.map((f: any) => [f.id, f.name]));

      let filtered = allTemplates.filter((t: any) => !t.deletedAt);
      if (user.role === "client") {
        filtered = filtered.filter((t: any) => t.isPublic === true);
      }

      const enhanced = filtered.map((t: any) => ({
        ...t,
        toolkitFolderName: t.toolkitFolderId ? (folderMap.get(t.toolkitFolderId) || null) : null,
      }));

      res.json(enhanced);
    } catch (error) {
      console.error("Toolkit templates error:", error);
      res.status(500).json({ error: "Failed to fetch toolkit templates" });
    }
  });
